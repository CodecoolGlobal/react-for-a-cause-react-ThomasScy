...
export default function Donate() {
	...
	const [donateDivClass, setDonateDivClass] = useState(
		"donate-div-donation1"
	);
	const [donateOptionsState, setDonateOptionsState] = useState("");
	const [bikerAnimation, setBikerAnimation] = useState("");
	const [bikerIconPosition, setBikerIconPosition] = useState(
		"donate-button-right"
	);
	...
	document.body.style.overflowY =
		...
		donateDivClass === "donate-div-donation5"
			? "hidden"
			: "";
	...
	function handleClick() {
		...
		setWaitingAnimation("");
		donateDivClass === "donate-div-donation1" ||
		donateDivClass === "donate-div-donation2"
			? donationMoveRight()
			: donationMoveLeft();
		setTimeout(
	...
	return (
		<div
			id="donate-section"
			className={`donate-div ${donateDivClass}`}
			style={{ display: formRootDisplay }}
		>
			<div className={`donate-options-container ${donateOptionsState}`}>
				...
				<div
					className="donate-submitted-container"
					style={{ display: thanksDisplay }}
				>
					<h1>Thank you!</h1>
					<h1 id="adios" style={{ visibility: ciaoVisibility }}>
						SEE YA!
					</h1>
				</div>
			</div>
			<div className="donate-button-root">
				<div
					id="donate-button"
					className={`${bikerAnimation} ${bikerIconPosition} ${waitingAnimation}`}
					onClick={handleClick}
				></div>
			</div>
		</div>
